...
type PanelKind = "training" | "camp" | "competition";

type CalendarNavViewProps = {
  camps: Camp[];
  error?: string | null;
  isMutating?: boolean;
...
const panelKinds: PanelKind[] = ["training", "camp", "competition"];

function getCampTypeLabel(type: Camp["type"]) {
  return type === "tävling" ? "Tävling" : "Läger";
}

function getPanelKindLabel(kind: PanelKind) {
  if (kind === "training") {
    return "Träning";
  }
...
function compactSummary(session: TrainingSession | null, camps: Camp[]) {
  const items: string[] = [];

  if (session) {
    items.push("1 träning");
  }

  const campCount = camps.filter((camp) => camp.type === "läger").length;
  const competitionCount = camps.filter((camp) => camp.type === "tävling").length;

  if (campCount > 0) {
    items.push(`${campCount} läger`);
  }

  if (competitionCount > 0) {
    items.push(`${competitionCount} tävlingar`);
  }

  return items.join(" • ");
}

export function CalendarNavView({
  camps,
  error = null,
  isMutating = false,
  onCreateCamp,
  ...
  onUpdateCamp,
  sessions,
}: CalendarNavViewProps) {
  const navigate = useNavigate();
  const today = getTodayValue();
  const [monthDate, setMonthDate] = useState(() => startOfMonth(new Date()));
  const [selectedDate, setSelectedDate] = useState(today);
  const [panelKind, setPanelKind] = useState<PanelKind>("training");
  ...
  useEffect(() => {
    resetQuickForm("training");
  }, [selectedDate]);

  function handleEditTraining() {
    setPanelKind("training");
  ...
  function handleEditCamp(camp: Camp) {
    setPanelKind(camp.type === "tävling" ? "competition" : "camp");
    ...
    try {
      if (panelKind === "training") {
        ...
        const payload: CampMutationInput = {
          ...
          type: panelKind === "competition" ? "tävling" : "läger",
        };
    ...
    <DashboardPage title="Kalender">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
        ...
        <div className="panel sticky top-0 self-start rounded-[20px] px-5 py-5">
          ...
          <div className="mt-1 text-[13px] text-[color:var(--ink2)]">
            {compactSummary(selectedSummary.session, selectedSummary.camps) ||
              "Inget planerat än på detta datum."}
          </div>
            ...
            <div className="space-y-2.5">
              ...
              {!selectedSummary.session && selectedSummary.camps.length === 0 ? (
                <div className="rounded-[14px] border border-dashed border-[color:var(--border)] px-4 py-4 text-[12px] leading-6 text-[color:var(--ink3)]">
                  Ingen aktivitet sparad än. Välj typ nedan och lägg till direkt
                  härifrån.
                </div>
              ) : null}
            </div>
            ...
            <div className="mt-5 grid gap-2 sm:grid-cols-2">
              ...
              >
                {isSubmitting
                  ...
                  : isEditingTraining || editingCampId
                    ? "Spara ändringar"
                    : `Lägg till ${getPanelKindLabel(panelKind).toLowerCase()}`}
              </button>
              <button
