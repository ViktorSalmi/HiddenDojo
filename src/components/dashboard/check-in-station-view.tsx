...
function parseSessionNotes(notes: string) {
  return notes
    .split(/\n\s*\n/)
...
export function CheckInStationView({
  ...
  const isStudentMode = isKiosk || mode === "student";
  ...
  async function handleOpen() {
    ...
    try {
      ...
      setMode("student");
      navigate("/dashboard/check-in?kiosk=1", { replace: true });
    } finally {
      setIsOpening(false);
    }
  }
  ...
  function renderMemberCards(columnsClassName: string, cardClassName: string) {
    return (
          ...
          filteredMembers.map((member) => {
            ...
            return (
              <button
                ...
                className={`${cardClassName} group relative rounded-[22px] border px-5 py-5 text-left transition-all ${
                  isCheckedIn
  ...
  if (isKiosk) {
    return (
      <div className="dojo-scrollbar flex h-full min-h-screen flex-col overflow-y-auto bg-[radial-gradient(circle_at_top,#fff8f2_0%,#f7f2ea_46%,#f3ede3_100%)]">
        <div className="mx-auto flex w-full max-w-[1380px] flex-1 flex-col px-6 py-6 sm:px-8 lg:px-10">
          <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
            <div>
              <div className="section-label">Incheckningsstation</div>
              <div className="mt-2 text-[14px] text-[color:var(--ink2)]">
                Sök ditt namn och tryck en gång för att checka in.
              </div>
            </div>
            <button
              className="ui-button-secondary rounded-[14px] px-4 py-3 text-[14px] font-medium text-[color:var(--ink2)]"
          ...
          <div className="panel rounded-[30px] px-6 py-6 sm:px-8 sm:py-8">
            <div className="mx-auto max-w-[920px] text-center">
              ...
              <div className="display-font mt-3 text-[40px] font-extrabold leading-tight text-[color:var(--ink)] sm:text-[54px]">
                Checka in dig här
              </div>
              <div className="mt-3 text-[16px] text-[color:var(--ink2)] sm:text-[18px]">
            ...
            <div className="mx-auto mt-8 max-w-[860px]">
              <div className="relative">
                <input
                  ref={searchInputRef}
                  autoComplete="off"
                  className="ui-input w-full rounded-[22px] px-6 py-5 pl-15 text-[20px] outline-none sm:text-[24px]"
                ...
                <svg
                  viewBox="0 0 24 24"
                  className="pointer-events-none absolute top-1/2 left-5 h-6 w-6 -translate-y-1/2 fill-none stroke-[var(--ink3)] stroke-2"
                >
          ...
          {error ? (
            <div className="mx-auto mt-5 w-full max-w-[860px] rounded-[16px] border border-[color:var(--red)] bg-[var(--red-pale)] px-4 py-3 text-[15px] text-[color:var(--red)]">
              {error}
            </div>
          ) : null}
        ...
        {confirmationName ? (
          <div className="pointer-events-none fixed inset-0 z-[400] flex items-start justify-center px-4 pt-8 sm:pt-10">
            <div className="w-full max-w-[520px] rounded-[28px] bg-[linear-gradient(180deg,#3a9a66_0%,var(--green)_100%)] px-7 py-6 text-white shadow-[0_28px_60px_rgba(45,122,79,0.28)] ring-1 ring-white/15">
    ...
    <DashboardPage title="Checka in">
      {!isStudentMode ? (
        <div className="panel mx-auto max-w-[880px] rounded-[24px] px-8 py-10 text-center">
          ...
          {todaySession?.notes ? (
            <div className="mx-auto mt-4 max-w-[58ch] rounded-[18px] bg-white/70 px-5 py-4 text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.85)]">
              ...
              <div className="mt-4 space-y-4">
                {parseSessionNotes(todaySession.notes).map((section) => (
                  <div key={section.heading}>
                    <div className="text-[13px] font-semibold text-[color:var(--ink)]">
                      {section.heading}
                    </div>
                    <ul className="mt-2 space-y-1.5 text-[14px] leading-6 text-[color:var(--ink2)]">
                      {section.items.map((item) => (
                        <li key={item} className="flex gap-2">
                          <span className="mt-[7px] h-1.5 w-1.5 shrink-0 rounded-full bg-[color:var(--green)]" />
                          <span>{item}</span>
                        </li>
                      ))}
                    </ul>
          ...
          <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
            <div className="ui-button-pill rounded-full px-4 py-2 text-[13px] font-medium text-[color:var(--ink2)]">
              {todaySession
                ? `${todaySession.attendee_ids.length} redan incheckade`
                : "Inget pass öppnat än"}
            </div>
          </div>
          {error ? (
        ...
        <div className="mx-auto flex w-full max-w-[1100px] flex-col gap-6">
          ...
          {renderMemberCards("grid gap-4 sm:grid-cols-2 xl:grid-cols-3", "")}
